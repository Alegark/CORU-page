...
- Entregas de STOCK: selector PERSONAL/Yummy/Nacional; puntos personales activos ordenados con mapa provider-neutral y lista accesible; administración CRUD/reordenamiento/archivado en `/admin/entregas`; MRW/ZOOM con estado, ciudad y oficina opcional; WhatsApp conserva el destino y `Cobro a destino`.
...
- Servicios de dominio para inventario atómico, tasa Bs (modo manual/automático y expiración Caracas), ajustes, categorías/productos e imágenes con adaptadores R2/Photoroom.
...
- El editor de productos conserva el historial de stock mediante movimientos `MANUAL_SET`; el pipeline de imágenes permite reintentar desde el original y aprobar original o procesada.
...
El adaptador Photoroom está integrado, pero `PHOTOROOM_API_KEY` todavía no está
configurada como secreto de Wrangler; mientras tanto el flujo conserva el
original y permite aprobarlo manualmente.
