...
import { ApiClientError, archiveAdminCategory, archiveAdminDeliveryPoint, archiveAdminProduct, archiveAdminPromotion, cancelAdminOrder, createAdminCategory, createAdminDeliveryPoint, createAdminProduct, createAdminPromotion, confirmAdminOrder, discardAdminOrder, fetchAdminAnalytics, fetchAdminCategories, fetchAdminDeliveryPoints, fetchAdminProducts, fetchAdminPromotions, fetchAdminRate, fetchAdminSettings, fetchAdminTraffic, markAdminPreorderDelivered, markAdminPreorderReady, recordAdminPreorderBalance, recordAdminPreorderDeposit, refreshAdminOrderRate, refreshAdminRate, reorderAdminCategories, reorderAdminDeliveryPoints, updateAdminCategory, updateAdminDeliveryPoint, updateAdminProduct, updateAdminRate, updateAdminSettings, uploadAdminImage, type AdminAnalyticsSummary, type AdminDeliveryPointInput, type AdminRateResponse } from '../../api/admin'
...
import { centimetersToMillimeters, deriveRingMeasurements, millimetersToCentimeters } from '../../../shared/ring-size'
...
function CoruSwitch({ label, checked, disabled = false, onChange }: { label: string; checked: boolean; disabled?: boolean; onChange: (checked: boolean) => void }) {
  return <label className={`checkbox-label coru-switch-field${disabled ? ' is-disabled' : ''}`}>
    <span className="coru-switch-copy">{label}</span>
    <span className="coru-switch">
      <input className="coru-switch-input" type="checkbox" checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} />
      <span className="coru-switch-track" aria-hidden="true"><span className="coru-switch-thumb" /></span>
    </span>
  </label>
}

function AdminProductArtwork({ product }: { product: Product }) {
  return <RingArtwork artwork={product.artwork} label={product.name} imageUrl={product.imageUrl} imageSource={product.imageSources?.[0]} />
}
...
type ProductDraft = {
  ...
  usSize: string
  circumferenceSource: 'auto' | 'manual'
  usSizeSource: 'auto' | 'manual'
}

function emptyProductDraft(category = 'Anillos'): ProductDraft {
  return { name: '', category, price: '4', stock: '0', sizeLabel: 'Talla única', material: 'Acero inoxidable', description: '', artwork: 'orbita', promoEligible: true, active: true, fulfillmentType: 'STOCK', measurementsText: '', leadTime: '3–4 semanas', innerDiameter: '', circumference: '', usSize: '', circumferenceSource: 'auto', usSizeSource: 'auto' }
}

function draftFromProduct(product: Product): ProductDraft {
  let circumferenceSource: ProductDraft['circumferenceSource'] = product.circumferenceMm === undefined ? 'auto' : 'manual'
  let usSizeSource: ProductDraft['usSizeSource'] = product.usSize === undefined ? 'auto' : 'manual'
  if (product.innerDiameterMm !== undefined && Number.isFinite(product.innerDiameterMm) && product.innerDiameterMm > 0) {
    const derived = deriveRingMeasurements(product.innerDiameterMm)
    if (product.circumferenceMm === derived.circumferenceMm) circumferenceSource = 'auto'
    if (product.usSize === derived.usSize) usSizeSource = 'auto'
  }
  return { name: product.name, category: product.category, price: String(product.priceCents / 100), stock: String(product.stockQuantity), sizeLabel: product.sizeLabel, material: product.material, description: product.description, artwork: product.artwork, promoEligible: product.promoEligible, active: product.active, fulfillmentType: product.fulfillmentType ?? 'STOCK', measurementsText: product.measurementsText ?? '', leadTime: product.leadTime ?? '3–4 semanas', innerDiameter: product.innerDiameterMm === undefined ? '' : String(millimetersToCentimeters(product.innerDiameterMm)), circumference: product.circumferenceMm === undefined ? '' : String(millimetersToCentimeters(product.circumferenceMm)), usSize: product.usSize ?? '', circumferenceSource, usSizeSource }
}
...
function ProductsPageV2({ products, setProducts }: { products: Product[]; setProducts: (products: Product[]) => void }) {
  ...
  function handleDiameterChange(value: string): void {
    const next = { ...draft, innerDiameter: value }
    const diameterCm = Number(value.replace(',', '.'))
    if (!value.trim()) {
      if (draft.circumferenceSource === 'auto') next.circumference = ''
      if (draft.usSizeSource === 'auto') next.usSize = ''
    } else if (Number.isFinite(diameterCm) && diameterCm > 0) {
      const derived = deriveRingMeasurements(centimetersToMillimeters(diameterCm))
      if (draft.circumferenceSource === 'auto') next.circumference = String(millimetersToCentimeters(derived.circumferenceMm))
      if (draft.usSizeSource === 'auto') next.usSize = derived.usSize ?? ''
    }
    setDraft(next)
  }

  function recalculateMeasurements(): void {
    const diameterCm = Number(draft.innerDiameter.replace(',', '.'))
    if (!Number.isFinite(diameterCm) || diameterCm <= 0) {
      setEditorError('Escribe un diámetro interno mayor que cero para recalcular.')
      return
    }
    const derived = deriveRingMeasurements(centimetersToMillimeters(diameterCm))
    setDraft({ ...draft, circumference: String(millimetersToCentimeters(derived.circumferenceMm)), usSize: derived.usSize ?? '', circumferenceSource: 'auto', usSizeSource: 'auto' })
    setEditorError('')
  }
  ...
  async function saveProduct(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!name || !Number.isInteger(priceCents) || priceCents < 0 || !Number.isInteger(stockQuantity) || stockQuantity < 0) { setEditorError('Completa nombre, precio y stock con valores válidos.'); return }
    const slug = name.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '')
    const innerDiameterCm = draft.innerDiameter.trim() ? Number(draft.innerDiameter.replace(',', '.')) : undefined
    const circumferenceCm = draft.circumference.trim() ? Number(draft.circumference.replace(',', '.')) : undefined
    const sizeLabel = draft.sizeLabel.trim() || 'Talla única'
    const measurementsText = draft.measurementsText.trim() || undefined
    const validInnerDiameterMm = Number.isFinite(innerDiameterCm) && innerDiameterCm! > 0 ? centimetersToMillimeters(innerDiameterCm!) : undefined
    const validCircumferenceMm = Number.isFinite(circumferenceCm) && circumferenceCm! > 0 ? centimetersToMillimeters(circumferenceCm!) : undefined
    const usSize = draft.usSize.trim() || undefined
    const input = { name, slug: slug || undefined, category: draft.category, priceCents, stockQuantity, sizeLabel, material: draft.material.trim(), description: draft.description.trim(), artwork: draft.artwork, promoEligible: draft.fulfillmentType === 'PREORDER' ? false : draft.promoEligible, active: draft.active, fulfillmentType: draft.fulfillmentType, measurementsText: currentEditingId ? measurementsText ?? null : measurementsText, innerDiameterMm: currentEditingId ? validInnerDiameterMm ?? null : validInnerDiameterMm, circumferenceMm: currentEditingId ? validCircumferenceMm ?? null : validCircumferenceMm, usSize: currentEditingId ? usSize ?? null : usSize, leadTime: draft.fulfillmentType === 'PREORDER' ? draft.leadTime.trim() || '3–4 semanas' : currentEditingId ? null : undefined }
    setSaving(true)
    try {
      try {
        const remote = currentEditingId ? await updateAdminProduct(currentEditingId, input) : await createAdminProduct(input)
        let nextProducts = currentEditingId ? products.map((product) => product.id === currentEditingId ? remote : product) : [...products, remote]
        let remainingImages = [...pendingImages]
        if (remainingImages.length) {
          try {
            for (const file of remainingImages) {
              await uploadAdminImage(remote.id, file)
              remainingImages = remainingImages.slice(1)
              setPendingImages(remainingImages)
            }
            try {
              const refreshed = await fetchAdminProducts()
              if (refreshed.length) nextProducts = refreshed
            } catch { /* the next refresh will sync the image variants */ }
          } catch (uploadError) {
            setProducts(nextProducts)
            setEditingId(remote.id)
            setDraft(draftFromProduct(remote))
            setEditorError(uploadError instanceof ApiClientError ? `Producto guardado, pero no se pudieron subir todas las imágenes: ${uploadError.message}` : 'Producto guardado, pero no se pudieron subir todas las imágenes. Puedes reintentar aquí.')
            return
          }
        }
        setProducts(nextProducts); setPendingImages([]); setEditorOpen(false); return
      } catch (error) {
      ...
      const local: Product = { id: currentEditingId ?? `product-${Date.now()}`, slug, name, category: draft.category, priceCents, stockQuantity, active: draft.active, primaryImageApproved: currentEditingId ? products.find((product) => product.id === currentEditingId)?.primaryImageApproved ?? false : false, promoEligible: draft.fulfillmentType === 'PREORDER' ? false : draft.promoEligible, fulfillmentType: draft.fulfillmentType, measurementsText, innerDiameterMm: validInnerDiameterMm, circumferenceMm: validCircumferenceMm, usSize, leadTime: draft.fulfillmentType === 'PREORDER' ? draft.leadTime.trim() || '3–4 semanas' : undefined, artwork: draft.artwork, description: draft.description.trim(), material: draft.material.trim(), sizeLabel }
      setProducts(currentEditingId ? products.map((product) => product.id === currentEditingId ? { ...product, ...local, id: product.id, slug: slug || product.slug } : product) : [...products, local]); setPendingImages([]); setEditorOpen(false)
    } finally { setSaving(false) }
  }

  async function removeProduct(product: Product): Promise<void> {
    if (!confirmDestructiveAction(`¿Eliminar el producto “${product.name}”? Esta acción no se puede deshacer.`)) return
    try {
      await archiveAdminProduct(product.id)
      setProducts(products.filter((entry) => entry.id !== product.id))
      setProductError('')
    } catch (requestError) {
      if (!isLocalPreviewError(requestError)) {
        setProductError(requestError instanceof ApiClientError ? requestError.message : 'No se pudo eliminar el producto.')
        return
      }
      setProducts(products.filter((entry) => entry.id !== product.id))
    }
  ...
  function renderEditor(): JSX.Element {
    return <form className="admin-panel form-panel product-editor product-editor-inline" onSubmit={(event) => void saveProduct(event)}>
      <div className="panel-header"><div><span className="eyebrow">{editingId ? 'Editar pieza' : 'Nueva pieza'}</span><h2>{editingId ? 'Actualiza los datos del producto' : 'Agrega una pieza al catálogo'}</h2></div><button className="button button-ghost" type="button" onClick={() => { setEditorOpen(false); setPendingImages([]) }} disabled={saving}>Cancelar</button></div>
      {editorError && <div className="inline-notice is-error" role="alert">{editorError}</div>}
      <div className="form-grid"><label>Nombre<input className="input" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} required /></label><label>Categoría<select className="input" value={draft.category} onChange={(event) => setDraft({ ...draft, category: event.target.value })}>{categories.filter((category) => category.active).map((category) => <option key={category.id} value={category.name}>{category.name}</option>)}</select></label><label>Precio USD<input className="input" value={draft.price} onChange={(event) => setDraft({ ...draft, price: event.target.value })} inputMode="decimal" /></label><label>Stock<input className="input" type="number" min="0" step="1" value={draft.stock} onChange={(event) => setDraft({ ...draft, stock: event.target.value })} /></label><label>Material<input className="input" value={draft.material} onChange={(event) => setDraft({ ...draft, material: event.target.value })} /></label><label>Modalidad<select className="input" value={draft.fulfillmentType} onChange={(event) => setDraft({ ...draft, fulfillmentType: event.target.value as NonNullable<Product['fulfillmentType']> })}><option value="STOCK">Disponible en stock</option><option value="PREORDER">Bajo pedido</option></select></label>{draft.fulfillmentType === 'PREORDER' && <label>Tiempo estimado<input className="input" value={draft.leadTime} onChange={(event) => setDraft({ ...draft, leadTime: event.target.value })} placeholder="3–4 semanas" /></label>}</div>
      <div className="form-grid ring-measurements"><label>Talla US<input className="input" value={draft.usSize} onChange={(event) => setDraft({ ...draft, usSize: event.target.value, usSizeSource: 'manual' })} inputMode="decimal" placeholder="Ej. 7" /><small>Se calcula desde el diámetro; puedes modificarla.</small></label><label>Diámetro interno (cm)<input className="input" value={draft.innerDiameter} onChange={(event) => handleDiameterChange(event.target.value)} inputMode="decimal" type="number" min="0" step="0.01" placeholder="Ej. 1.73" /><small>Dato base en centímetros.</small></label><label>Circunferencia (cm)<input className="input" value={draft.circumference} onChange={(event) => setDraft({ ...draft, circumference: event.target.value, circumferenceSource: 'manual' })} inputMode="decimal" type="number" min="0" step="0.01" placeholder="Ej. 5.44" /><small>Se calcula automáticamente; puedes modificarla.</small></label><button className="button button-ghost" type="button" onClick={recalculateMeasurements}>Recalcular desde diámetro</button></div>
      <label>Descripción<textarea className="textarea" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} /></label><CoruSwitch label="Visible en el catálogo" checked={draft.active} onChange={(active) => setDraft({ ...draft, active })} /><CoruSwitch label="Elegible para la promoción" checked={draft.promoEligible} disabled={draft.fulfillmentType === 'PREORDER'} onChange={(promoEligible) => setDraft({ ...draft, promoEligible })} />
      <ImageProcessor productId={editingId ?? undefined} pendingFiles={pendingImages} onPendingFilesChange={setPendingImages} disabled={saving} onUpload={editingId ? async (file) => { const uploaded = await uploadAdminImage(editingId, file); try { setProducts(await fetchAdminProducts()) } catch { /* keep current product list */ } return uploaded } : undefined} onChanged={editingId ? async () => { try { setProducts(await fetchAdminProducts()) } catch { /* keep current product list */ } } : undefined} />
      <button className="button button-primary" type="submit" disabled={saving}><Icon icon={icons.check} /> {saving ? 'Guardando…' : 'Guardar producto'}</button>
    </form>
  }

  return <><PageHead kicker="Catálogo" title="Productos" copy="Mantén cada pieza lista para salir a la calle." action={<button className="button button-primary" type="button" onClick={() => openEditor()}><Icon icon={icons.plus} /> Nuevo producto</button>} />{productError && <div className="inline-notice is-error" role="alert">{productError}</div>}
    <div className="admin-toolbar"><div className="search-field"><Icon icon={icons.search} /><input className="input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre…" aria-label="Buscar productos" /></div></div>
    {editorOpen && !editingId && <div className="admin-product-editor-slot admin-product-editor-new">{renderEditor()}</div>}
    <section className="admin-panel admin-product-panel"><div className="admin-product-list">
      <div className="admin-product-list-head" aria-hidden="true"><span>Producto</span><span>Categoría</span><span>Modalidad</span><span>Precio</span><span>Stock</span><span>Estado</span><span /></div>
      {filtered.map((product) => <Fragment key={product.id}><article className="admin-product-row"><div className="admin-product-main"><div className="table-product"><AdminProductArtwork product={product} /><span><strong>{product.name}</strong><small>{product.sizeLabel}</small></span></div><div className="admin-product-mobile-meta"><span>{product.category} · {product.fulfillmentType === 'PREORDER' ? 'Bajo pedido' : formatUsd(product.priceCents)}</span><small>{product.fulfillmentType === 'PREORDER' ? `${product.leadTime ?? '3–4 semanas'}` : product.stockQuantity === 0 ? 'Agotado' : `${product.stockQuantity} en stock`}</small></div></div><div className="admin-product-cell">{product.category}</div><div className="admin-product-cell">{product.fulfillmentType === 'PREORDER' ? <span className="status-badge status-warning"><span />Bajo pedido</span> : <span className="status-badge status-confirmed"><span />Stock</span>}</div><div className="admin-product-cell">{formatUsd(product.priceCents)}</div><div className="admin-product-cell"><span className={`stock-value${product.stockQuantity <= 3 && product.fulfillmentType !== 'PREORDER' ? ' is-low' : ''}`}>{product.fulfillmentType === 'PREORDER' ? '—' : product.stockQuantity}</span></div><div className="admin-product-cell"><span className={`status-badge ${product.active ? 'status-confirmed' : 'status-discarded'}`}><span />{product.active ? 'Visible' : 'Oculto'}</span></div><div className="product-actions"><button className="icon-button" type="button" aria-label={`Editar ${product.name}`} onClick={() => openEditor(product)}><Icon icon={icons.pen} /></button>{!product.active && <button className="icon-button" type="button" aria-label={`Eliminar ${product.name}`} title="Eliminar producto" onClick={() => void removeProduct(product)}><Icon icon={icons.trash} /></button>}</div></article>{editorOpen && editingId === product.id && <div className="admin-product-editor-slot">{renderEditor()}</div>}</Fragment>)}
      {filtered.length === 0 && <div className="admin-empty"><Icon icon={icons.box} /><h3>No hay productos que coincidan</h3><p>Prueba con otro nombre.</p></div>}
    </div></section></>
}
  ...
  async function updateStatus(status: OrderStatus) {
    ...
    try {
      try {
        ...
        setOrders(updated)
        if (isVitePreview()) saveOrders(updated)
  ...
  async function refreshRate() {
    ...
    try {
      try {
        ...
        setOrders(refreshed)
        if (isVitePreview()) saveOrders(refreshed)
...
function localAnalyticsSummary(orders: Order[]): AdminAnalyticsSummary {
  ...
  return {
    ...
    traffic: { visits: 0, pageViews: 0, sessions: 0, uniqueVisitors: 0, pagesPerSession: 0 },
