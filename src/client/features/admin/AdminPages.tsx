...
export function AdminPages({ section, orderId, products, setProducts, orders, setOrders }: AdminPagesProps) {
  ...
  if (section === 'analytics') return <AnalyticsPageLive orders={orders} />
...
const seededDeliveryPoint: PersonalDeliveryPoint = { id: 'coru-punto-central', name: 'Punto CORU · Centro', address: 'Punto coordinado por CORU', shortDescription: 'Entrega personal en Maracaibo', scheduleText: 'Coordina por WhatsApp', active: true, sortOrder: 1 }
...
function OrderDetailV2({ order, products, setProducts, orders, setOrders }: { order: Order; products: Product[]; setProducts: (products: Product[]) => void; orders: Order[]; setOrders: (orders: Order[]) => void }) {
  ...
  const destinationLabel = order.shipping?.method === 'PERSONAL' ? `${order.shipping.deliveryPointName ?? order.shipping.deliveryPointId}${order.shipping.deliveryPointAddress ? ` · ${order.shipping.deliveryPointAddress}` : ''}` : order.shipping?.method === 'YUMMY' ? order.shipping.addressText : order.shipping?.method === 'NATIONAL' ? `${order.shipping.state}, ${order.shipping.city}${order.shipping.officeText ? ` · ${order.shipping.officeText}` : ''}` : undefined
