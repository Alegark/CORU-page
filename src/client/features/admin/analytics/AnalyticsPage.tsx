...
import { ApiClientError, clearAdminCartAddAnalytics, fetchAdminAnalyticsSummary, fetchAdminProductAnalytics, type AdminAnalyticsProductInterest, type AdminAnalyticsRange, type AdminAnalyticsSummaryV2 } from '../../../api/admin'

type Period = 'today' | '3d' | '7d' | '30d' | 'month' | 'custom'
type TimelineMetric = 'uniqueVisitors' | 'unitsAdded' | 'whatsappIntents'
...
function sourceLabel(source: AdminAnalyticsSummaryV2['sources'][number]['source']): string {
  return source === 'instagram' ? 'Instagram' : source === 'facebook' ? 'Facebook' : source === 'whatsapp' ? 'WhatsApp' : source === 'search' ? 'Buscadores' : source === 'direct' ? 'Directo' : 'Otros'
}
...
function AnalyticsTimeline({ summary }: { summary: AdminAnalyticsSummaryV2 }) {
  const [metric, setMetric] = useState<TimelineMetric>('uniqueVisitors')
  const metricLabels: Record<TimelineMetric, string> = { uniqueVisitors: 'Visitantes únicos por día', unitsAdded: 'Agregados', whatsappIntents: 'WhatsApp' }
...
export function AnalyticsPage() {
  ...
  const [clearingCartAdds, setClearingCartAdds] = useState(false)
  const [confirmingCartAdds, setConfirmingCartAdds] = useState(false)
  ...
  const hasActivity = Boolean(summary && (summary.kpis.pageViews || summary.kpis.sessions || summary.kpis.uniqueVisitors || summary.kpis.unitsAdded || summary.kpis.whatsappIntents || summary.realOrders.pending || summary.realOrders.confirmed || summary.realOrders.discarded || summary.realOrders.cancelled))
  const funnel = useMemo(() => summary ? [['Visitó la tienda', summary.funnel.catalogSessions], ['Producto visto', summary.funnel.productViewSessions], ['Agregó al carrito', summary.funnel.addSessions], ['Intento por WhatsApp', summary.funnel.whatsappSessions], ['Venta concretada', summary.funnel.confirmedOrders]] as const : [], [summary])
  ...
  async function clearCartAddAnalytics(): Promise<void> {
    if (clearingCartAdds) return
    setClearingCartAdds(true)
    setError('')
    setNotice('')
    try {
      const result = await clearAdminCartAddAnalytics()
      setNotice(`Se eliminaron ${result.deletedEvents.toLocaleString('es-VE')} eventos de agregado al carrito. Las demás métricas se conservaron.`)
      setConfirmingCartAdds(false)
      setReloadKey((key) => key + 1)
    } catch (requestError) {
      setError(requestError instanceof ApiClientError ? requestError.message : 'No se pudieron borrar las estadísticas de agregado al carrito.')
    } finally {
      setClearingCartAdds(false)
    }
  }

  return <div className="analytics-page">
    <div className="admin-page-head"><div><span className="eyebrow">Señales anónimas</span><h1>Actividad del catálogo</h1><p>Visitas, productos consultados e intención de compra.</p><p className="admin-panel-note">No incluye visitas de navegadores que abrieron este panel.</p></div><span className="analytics-retention-note">Datos anónimos · retención 180 días</span></div>
    <section className="analytics-toolbar" aria-label="Filtrar período"><div className="analytics-periods">{periodLabels.map((item) => <button key={item.value} type="button" className={period === item.value ? 'is-active' : ''} onClick={() => choosePeriod(item.value)}>{item.label}</button>)}<button type="button" className={period === 'custom' ? 'is-active' : ''} onClick={() => choosePeriod('custom')}>Personalizado</button></div><div className="analytics-toolbar-actions"><button className="button button-secondary analytics-refresh" type="button" onClick={retry} disabled={refreshing || clearingCartAdds}><Icon icon={icons.rotate} /> {refreshing ? 'Actualizando…' : 'Actualizar'}</button><button className="button button-danger analytics-clear-cart-adds" type="button" onClick={() => setConfirmingCartAdds(true)} disabled={refreshing || clearingCartAdds}>{clearingCartAdds ? 'Borrando…' : <><Icon icon={icons.trash} /> Limpiar agregados</>}</button></div></section>
    {confirmingCartAdds && <section className="analytics-clear-confirmation" role="alertdialog" aria-labelledby="analytics-clear-title" aria-describedby="analytics-clear-description"><div><strong id="analytics-clear-title">¿Borrar los agregados al carrito?</strong><p id="analytics-clear-description">Solo se eliminarán esos eventos de analítica; visitas, consultas de productos, pedidos y ventas se conservarán. No se puede deshacer.</p></div><div className="analytics-clear-confirmation-actions"><button className="button button-secondary" type="button" onClick={() => setConfirmingCartAdds(false)} disabled={clearingCartAdds}>Cancelar</button><button className="button button-danger" type="button" onClick={() => void clearCartAddAnalytics()} disabled={clearingCartAdds}>{clearingCartAdds ? 'Borrando…' : 'Sí, borrar agregados'}</button></div></section>}
    ...
    {notice && <div className="inline-notice is-success" role="status" aria-live="polite">{notice}</div>}
