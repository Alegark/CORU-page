...
export function ProductPage({ product, products, promotion, onOrderCreated }: { product: Product; products: Product[]; promotion?: PromotionRule | null; onOrderCreated: (order: Order) => void }) {
  ...
  useEffect(() => { if (viewedProductRef.current === product.id) return; viewedProductRef.current = product.id; analytics.track('product_view', { productId: product.id, productName: product.name, category: product.category, unitPriceCents: product.priceCents, promoEligible: product.promoEligible, fulfillment_type: product.fulfillmentType ?? 'STOCK' }) }, [product.id, product.name, product.category, product.priceCents, product.promoEligible, product.fulfillmentType])
  useEffect(() => { setSelectedImage(gallery[0]) }, [product.id, product.imageUrl, product.imageUrls?.join('|')])
  return <div className="product-page app-shell"><header className="simple-header page-container"><Brand /><button className="icon-button" type="button" onClick={() => navigate('/')} aria-label="Volver a la colección"><Icon icon={icons.arrowLeft} /></button></header><main id="main-content" className="page-container product-detail"><button className="back-link" type="button" onClick={() => navigate('/')}><Icon icon={icons.arrowLeft} /> Volver a la colección</button><div className="product-detail-grid"><div className="product-detail-visual"><span className="product-badge">{promoLabel}</span><RingArtwork artwork={product.artwork} label={product.name} large imageUrl={selectedImage} />{gallery.length > 1 && <div className="product-gallery" aria-label="Galería del producto">{gallery.map((image, index) => <button className={`product-gallery-thumb${selectedImage === image ? ' is-active' : ''}`} type="button" key={image} onClick={() => setSelectedImage(image)} aria-label={`Ver imagen ${index + 1}`}><img src={image} alt="" /></button>)}</div>}</div><div className="product-detail-copy"><span className="eyebrow">{product.category} · CORU</span><h1 className="display-heading">{product.name}</h1><div className="detail-price-row"><strong>{formatCurrencyAmount(product.priceCents, currency, rateMicros)}</strong><span className="stock-pill">{isPreorder ? 'Bajo pedido' : product.stockQuantity <= 0 ? 'Agotado' : product.stockQuantity <= 3 ? 'Últimas piezas' : 'En stock'}</span></div><p className="detail-description">{product.description}</p>{isPreorder && <div className="preorder-notice"><strong>Tiempo estimado de llegada: {product.leadTime ?? '3–4 semanas'}</strong><span>El tiempo es estimado y puede variar.</span><span>50% al solicitar · 50% al entregar.</span></div>}<dl className="detail-meta"><div><dt>Material</dt><dd>{product.material}</dd></div><div><dt>Información</dt><dd>{product.sizeLabel}</dd></div>{product.measurementsText && <div><dt>Medidas</dt><dd>{product.measurementsText}</dd></div>}</dl><button className="button button-primary detail-add" type="button" disabled={!product.active || (!isPreorder && product.stockQuantity <= 0)} onClick={() => { add(product); setAdded(true) }}><Icon icon={added ? icons.check : icons.plus} /> {isPreorder ? added ? 'Agregado al carrito' : 'Solicitar bajo pedido' : product.stockQuantity <= 0 ? 'Agotado' : added ? 'Agregado al carrito' : 'Agregar al carrito'}</button><button className="detail-cart-link" type="button" onClick={() => navigate('/guia-de-tallas')}>Guía de tallas</button></div></div></main><FloatingCart itemCount={itemCount} open={cartOpen} onCart={() => setCartOpen(true)} /><CartOverlay products={products} open={cartOpen} onClose={() => setCartOpen(false)} onOrderCreated={onOrderCreated} rateMicros={rateMicros} rateAvailable={rateAvailable} /></div>
}
